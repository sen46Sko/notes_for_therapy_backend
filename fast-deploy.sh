...
# Zip current folder
zip -r deploy.zip . -x "*.git*" -x "*node_modules*" -x "*deploy.sh" -x "*.vercel*"

# # Cleanup previous deployment
# ssh -i KateLaravelSSH.pem ubuntu@3.124.219.120 << 'ENDSSH'
...
scp -i KateLaravelSSH.pem deploy.zip ubuntu@3.124.219.120:/var/www
...
# Move deploy.zip from /var/www to /var/www/KateLaravel
sudo mv /var/www/deploy.zip /var/www/KateLaravel
